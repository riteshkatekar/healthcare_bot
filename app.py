...
SUMMARY_TRIGGER_MESSAGES = int(os.getenv("SUMMARY_TRIGGER_MESSAGES", "50"))
...
def maybe_store_language_preference(session_id: str, user_message: str, language_hint: str) -> None:
    """
    Persist an explicit user language request such as:
    - In Marathi
    - मराठीत सांगा
    - Hindi me
    """
    ...
    explicit_language_phrases = {
        "मराठीत": "mr",
        "मराठीत सांगा": "mr",
        "मराठीत उत्तर द्या": "mr",
        "in marathi": "mr",
        "marathi": "mr",
        "हिंदी में": "hi",
        "हिंदी": "hi",
    ...
    chosen_lang = None
    for phrase, lang in explicit_language_phrases.items():
        if phrase in text:
            chosen_lang = lang
            break
    ...
    if chosen_lang:
        try:
            memory_store.set_user_preference(session_id, "preferred_language", chosen_lang)
            return
        except Exception:
            pass

    # If the user repeatedly uses one language, preserve it as a soft preference.
    if language_hint and language_hint != "mixed":
        try:
            memory_store.set_user_preference(session_id, "preferred_language", language_hint)
        except Exception:
            pass
...
def build_generation_context(
    ...
    *,
    ...
    stored_lang = None
    try:
        stored_lang = memory_store.get_user_preference(session_id, "preferred_language")
    except Exception:
        stored_lang = None

    if stored_lang:
        language_hint = stored_lang

    maybe_store_language_preference(session_id, user_message, language_hint)
...
def compose_answer(
    ...
    *,
    ...
    try:
        answer = groq_service.chat(messages, temperature=0.3, max_tokens=1400)
...
def stream_answer(
    *,
    ...
    def generate():
        ...
        try:
            for chunk in groq_service.stream_chat(messages, temperature=0.3, max_tokens=1400):
